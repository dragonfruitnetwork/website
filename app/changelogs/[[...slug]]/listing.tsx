...
export default async function Listing() {
    ...
    return (
        <div className="flex flex-row flex-wrap" style={{maxWidth: "100%", width: "max-content"}}>
            {publicApps.map((x) => (
                <a key={x.id} href={`/changelogs/${x.id}/${x.releaseName}`} className="block p-1 text-decoration-none w-[125px] hover:w-[150px] lg:hover:w-[175px] transition-all select-none" style={{willChange: "opacity"}}>
